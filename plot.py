...
def _read_report_summary(data_dir: Path) -> Dict[str, dict]:
    return {r["run"]: r for r in read_csv(data_dir / "report" / "summary.csv")}


def _primary_report_runs(summary: Dict[str, dict]) -> set:
    """Keep the method-comparison runs out of historical mixed report/PPC CSVs."""
    if not summary:
        return set()
    ...
    for row in summary.values():
        ...
        if row.get("status") == "ok" and row.get("method") != "pic" and ppc is not None:
            non_pic_ppcs.append(int(ppc))
    if not non_pic_ppcs:
        return set(summary.keys())

    primary_ppc = Counter(non_pic_ppcs).most_common(1)[0][0]
    selected = set()
    for run, row in summary.items():
        if row.get("status") != "ok":
            continue
        ppc = _try_float(row.get("ppc"))
        repeat = _try_float(row.get("repeat"))
        if ppc is None or int(ppc) != primary_ppc:
            continue
        if repeat is not None and int(repeat) != 1:
            continue
        selected.add(run)
    return selected or set(summary.keys())


def _ppc_summary_rows(data_dir: Path) -> Optional[List[dict]]:
    ppc_csv = data_dir / "ppc" / "summary.csv"
    if ppc_csv.exists():
        rows = _load_or_warn(ppc_csv, "ppc")
        if rows is not None:
            return rows

    fallback = data_dir / "report" / "summary.csv"
    rows = _load_or_warn(fallback, "ppc fallback")
    if rows is None:
        return None
    print("[plot] ppc: using legacy data/report/summary.csv fallback")
    return rows
...
def plot_energy(data_dir: Path, img_dir: Path, formats: tuple) -> None:
    ...
    run_meta = _read_report_summary(data_dir)
    allowed_runs = _primary_report_runs(run_meta)
...
def plot_velocity(data_dir: Path, img_dir: Path, formats: tuple) -> None:
    ...
    style_ax(ax, xlabel="Time [s]", ylabel="Particle vertical velocity [m/s]",
...
def _plot_ppc(
    ...
    rows = _ppc_summary_rows(data_dir)
    if rows is None:
        return
...
def plot_iterative(data_dir: Path, img_dir: Path, formats: tuple) -> None:
    ...
    for row in rows:
        ...
        label = f"{solver} ({threads} threads)" if threads not in ("", None) else solver
        ...
        by_solver[label].append((int(step), iters))
        if hit_max:
            hit_max_by_solver[label].append((int(step), iters))
